...
interface GlassyBannerProps {
  text?: string;
  Icon?: IconType;
}

const GlassyBanner = ({
  text = "v1 launching soon",
  Icon = LuSparkles,
}: GlassyBannerProps) => {
  return (
    <div className="inline-flex items-center gap-x-2 rounded-full border border-white/20 bg-white/10 px-4 py-1.5 sm:text-sm text-xs text-white/80 backdrop-blur-sm">
      ...
      <div className="relative z-10 flex flex-col items-center min-h-screen">
        ...
        <nav className="flex w-full max-w-7xl items-center justify-between px-6 py-4 ring-1 ring-neutral-800 rounded-full">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-base md:text-xl font-bold text-white">Mailfy</span>
          </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            <Link
              href="#"
              className="rounded-full text-sm bg-white px-5 py-2 font-medium text-black shadow-lg transition-transform hover:scale-105"
            >
              Sign Up
            </Link>
          </div>
        ...
        <main className="flex flex-1 flex-col items-center justify-center text-center mt-4 sm:mt-8">
          ...
          <h1 className="text-4xl font-bold leading-tight text-white sm:text-5xl md:text-7xl mt-6">
            ...
            <br className="hidden sm:block" /> meetings and deals
          </h1>
