export const mobileAccordionNavbarCode = `"use client";
...
export default function FullNavGlass() {
  ...
  return (
    <div className="w-full bg-white dark:bg-black">
      ...
      <nav className="hidden lg:flex items-center justify-between w-full h-20 px-8 bg-black/80 dark:bg-white/80 backdrop-blur-md border-b border-neutral-800/50 dark:border-neutral-200/50 sticky top-0 z-50">
        <DevStudioLogoInverted />
        <div className="flex items-center gap-x-8">
          <a
            href="#"
            className="text-neutral-300 dark:text-neutral-700 hover:text-white dark:hover:text-black transition-colors"
          >
            Services
          </a>
          <a
      ...
      <div className="lg:hidden w-full">
        ...
        <nav className="flex items-center justify-between w-full h-20 px-6 bg-white dark:bg-black border-b border-neutral-200 dark:border-neutral-800">
          <DevStudioLogo />
          <button onClick={toggleMenu} aria-label="Toggle menu" className="text-neutral-900 dark:text-white">
            {isMenuOpen ? <HiX className="w-7 h-7" /> : <TbMenu className="w-7 h-7" />}
          </button>
        </nav>
